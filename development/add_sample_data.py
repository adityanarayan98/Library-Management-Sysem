...
def add_sample_data():
    ...
    try:
        ...
        books = [
            ...
            ('1984', 'George Orwell', '9780451524935', 'Signet Classic', 1950, 'LIT002', 4, 'issued', 600.00),
        ...
        transactions = [
            (patron1_id, book1_id, date.today().strftime('%Y-%m-%d'), (date.today() + timedelta(days=14)).strftime('%Y-%m-%d'), None, 'issued', 0.0, 0, 1, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
            (patron2_id, book2_id, date.today().strftime('%Y-%m-%d'), (date.today() + timedelta(days=14)).strftime('%Y-%m-%d'), None, 'issued', 0.0, 0, 1, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        ]
